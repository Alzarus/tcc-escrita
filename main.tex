...
A arquitetura do \textit{Tô De Olho} foi projetada seguindo o padrão de \textbf{monolito modular} \cite{dragoni2017microservices}, escolhido por sua simplicidade de desenvolvimento e deploy, mantendo a organização interna em módulos bem definidos para garantir manutenibilidade. A seguir, justificamos cada escolha tecnológica.

\textbf{Backend --- Golang:}

A linguagem Go foi selecionada para o \textit{backend} ap\'os avalia\c{c}\~ao de alternativas como Node.js e Python. O fator decisivo foi o modelo de \textbf{concorr\^encia nativo}: as \textit{goroutines} permitem processar milhares de conex\~oes simult\^aneas com baixo consumo de mem\'oria, caracter\'istica essencial para a ingest\~ao paralela de dados de tr\^es APIs distintas \cite{nanz2015comparative}. Adicionalmente, a tipagem est\'atica e a compila\c{c}\~ao para bin\'ario \'unico simplificam o \textit{deploy} em cont\^eineres.

O framework \textbf{Gin} foi escolhido por seu roteamento HTTP de alto desempenho \cite{alfian2024gin}, enquanto \textbf{GORM} oferece mapeamento objeto-relacional com suporte a migracoes, reduzindo a complexidade de gerenciamento do esquema de banco.

\textbf{Banco de Dados --- PostgreSQL e Redis:}

\textbf{PostgreSQL} foi selecionado como banco relacional por sua robustez em consultas anal\'iticas e suporte a \'indices compostos \cite{postgresql2024}, essenciais para agrega\c{c}\~oes por senador, per\'iodo e tipo de despesa. A conformidade ACID garante integridade nas opera\c{c}\~oes de ingest\~ao.

\textbf{Redis} atua como camada de cache para rankings pr\'e-computados e totalizadores de gastos \cite{redis2024}. Com tempos de resposta em microssegundos, elimina a necessidade de recalcular m\'etricas a cada requisi\c{c}\~ao, atendendo ao requisito de lat\^encia inferior a 2 segundos (RNF01).

\textbf{Frontend --- Next.js 15:}

A escolha de \textbf{Next.js} sobre React ``puro'' justifica-se pela necessidade de \textbf{SEO} e performance inicial \cite{salim2024nextjs}. Aplica\c{c}\~oes React tradicionais utilizam renderiza\c{c}\~ao no cliente (CSR), onde motores de busca recebem uma p\'agina HTML vazia que depende de JavaScript para exibir conte\'udo --- prejudicando a indexa\c{c}\~ao. Next.js resolve esse problema com \textit{Server-Side Rendering} (SSR) e \textit{Static Site Generation} (SSG), entregando HTML pr\'e-renderizado aos \textit{crawlers}.

A biblioteca \textbf{Recharts} foi selecionada para visualiza\c{c}\~ao de dados por sua integra\c{c}\~ao nativa com React e suporte a gr\'aficos responsivos \cite{recharts2024}. \textbf{Tailwind CSS} permite estiliza\c{c}\~ao eficiente com classes utilit\'arias, reduzindo o tamanho do CSS final atrav\'es de \textit{tree-shaking} \cite{tailwindcss2024}.
