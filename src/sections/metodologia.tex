...
O modelo relacional foi projetado para garantir integridade referencial e eficiência em consultas analíticas. A Figura~\ref{fig:er-diagram} apresenta o diagrama entidade-relacionamento simplificado do sistema.

\begin{figure}[H]
\centering
\begin{verbatim}
+-------------+       +-------------+       +-------------+
|   Senador   |1     N|   Mandato   |       |   Votacao   |
+-------------+-------+-------------+       +-------------+
| PK id       |       | PK id       |       | PK id       |
| UK cod_parl |       | FK senador  |       | FK senador  |
| nome        |       | legislatura |       | FK sessao   |
| partido     |       | tipo        |       | voto        |
| uf          |       | inicio/fim  |       | orientacao  |
+------+------+       +-------------+       +-------------+
       |1
       +------------------+------------------+
       |N                 |N                 |N
+------+------+    +------+------+    +------+------+
| DespesaCEAPS|    |   Emenda    |    |  Servidor   |
+-------------+    +-------------+    +-------------+
| PK id       |    | PK id       |    | PK id       |
| FK senador  |    | FK senador  |    | FK senador  |
| ano/mes     |    | ano         |    | nome        |
| tipo_desp   |    | tipo        |    | cargo       |
| fornecedor  |    | localidade  |    | vinculo     |
| cpf_cnpj    |    | vlr_empenh  |    | remuneracao |
| valor       |    | vlr_pago    |    |             |
+-------------+    +-------------+    +-------------+
\end{verbatim}
...
Esta modelagem permite consultas analíticas eficientes, como ranking de senadores por economia na cota ou distribuição geográfica de emendas, sem necessidade de joins custosos em tempo de execução.
