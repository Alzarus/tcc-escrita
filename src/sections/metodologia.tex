...
Tarefas agendadas (\textit{cron jobs}) executam automaticamente no horário de menor tráfego das APIs (madrugada, horário de Brasília). A frequência de cada tarefa foi definida conforme a volatilidade dos dados:

\begin{itemize}
    \item \textbf{Diário}: Senadores em exercício, despesas CEAPS do mês corrente, votações dos últimos 7 dias e recálculo de rankings;
    \item \textbf{Semanal}: Remunerações de servidores de gabinete (dados atualizados mensalmente na fonte, mas verificados semanalmente);
    \item \textbf{Mensal}: Emendas parlamentares (consolidação ocorre no início de cada mês no Portal da Transparência).
\end{itemize}

A ordem de execução respeita dependências: a lista de senadores é sincronizada primeiro, pois os demais jobs dependem dos identificadores parlamentares atualizados. O recálculo de rankings executa por último, após a consolidação de todos os dados.

\subsubsection{Idempotência e Deduplicação}

Para garantir consistência em reprocessamentos e sincronizações incrementais, todas as operações de carga utilizam \textbf{upserts} (\textit{insert or update}). A estratégia baseia-se em chaves naturais únicas:

\begin{itemize}
    \item \textbf{Despesas CEAPS}: Chave composta \texttt{(senador\_id, ano, mês, fornecedor\_cnpj, valor)} --- identifica unicamente cada reembolso;
    \item \textbf{Votações}: Chave composta \texttt{(sessao\_id, senador\_id)} --- registra o voto de cada senador em cada sessão;
    \item \textbf{Emendas}: Código único da emenda no Portal da Transparência.
\end{itemize}

Essa abordagem permite reexecutar jobs de sincronização sem risco de duplicação de dados, simplificando a recuperação de falhas e facilitando o \textit{backfill} parcial quando necessário.

\subsubsection{Tratamento de Erros e Resiliência}

Para garantir robustez na ingestão, o sistema implementa mecanismos de resiliência em múltiplas camadas:

\begin{itemize}
    \item \textbf{Retry com Backoff Exponencial}: Em caso de falha de requisição (timeout, erro 5xx ou rate limit), o sistema aguarda 2, 4, 8 e 16 segundos entre tentativas, até um máximo de 5 retentativas;
    \item \textbf{Rate Limiting por Fonte}: Controle diferenciado de requisições conforme limites de cada API: 10 requisições por segundo para a API Legislativa, 5 requisições por segundo para a API Administrativa, e 5 requisições por segundo para o Portal da Transparência (300/minuto com margem de segurança);
    \item \textbf{Circuit Breaker}: Após 3 falhas consecutivas em uma fonte, o sistema suspende requisições por 5 minutos, evitando sobrecarga de APIs temporariamente indisponíveis;
    \item \textbf{Logging Estruturado}: Registros em formato JSON com campos padronizados (timestamp, operação, duração, status, erro) para integração com ferramentas de observabilidade.
\end{itemize}

\subsubsection{Monitoramento e Alertas}

A operação contínua do sistema requer visibilidade sobre a saúde dos processos de ingestão:

\begin{itemize}
    \item \textbf{Métricas de Execução}: Cada job registra duração, quantidade de registros processados e taxa de erro;
    \item \textbf{Alertas de Falha}: Notificações são disparadas quando um job falha após esgotar retentativas ou quando a defasagem de dados ultrapassa 48 horas;
    \item \textbf{Dashboard de Status}: Painel exibindo última sincronização bem-sucedida de cada fonte, permitindo identificação rápida de problemas.
\end{itemize}
