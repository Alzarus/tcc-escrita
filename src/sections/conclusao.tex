...
Este trabalho apresentou o \textit{Tô De Olho}, uma plataforma \textit{web} que centraliza e democratiza o acesso aos dados do Senado Federal. A arquitetura de \textbf{monolito modular} em \textit{Go}, combinada com a ingestão de dados via APIs oficiais, mostrou-se adequada para consolidar informações dispersas em três fontes distintas. O \textit{front-end} em \textit{Next.js} oferece ao cidadão uma interface acessível para fiscalizar despesas da CEAPS, acompanhar votações nominais e avaliar o desempenho dos 81 senadores por meio de uma classificação (\textit{ranking}) com metodologia transparente.
...
Para trabalhos futuros, almeja-se a implementação das funcionalidades mapeadas nos Requisitos mas não contempladas no Produto Mínimo Viável (MVP), a saber:
\begin{enumerate}
