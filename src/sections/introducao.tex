...
A literatura evidencia que TICs ampliam possibilidades de participação, mas só geram valor quando articuladas a contextos de uso. Avelino \textit{et al.} mapeiam iniciativas, reforçando que tecnologias exigem visualizações para o controle social \cite{avelino2021democracia}. Com um corpo legislativo menor e mais ``caro'' \textit{per capita} que a Câmara, o Senado carece de ferramentas que cruzem votações com a execução orçamentária. À luz desses estudos, o \textit{Tô De Olho} procura transformar transparência passiva em prestação de contas ativa \cite{pateman1970, gomes2019}.
...
\begin{enumerate}
    \item \textbf{Classificação Metodologicamente Fundamentada}: desenvolvido a partir do estudo da metodologia \textit{Legislative Effectiveness Score} (LES) de Volden e Wiseman (\citeyear{volden2014lawmakers}), o algoritmo de avaliação pondera produtividade legislativa (35\%), presença em votações (25\%), economia na cota parlamentar (20\%) e participação em comissões (20\%). Os critérios e pesos são públicos, permitindo ao cidadão compreender --- e questionar --- a metodologia;
    
    \item \textbf{Visualização Orientada à Ação}: seguindo os princípios de retórica visual de Hullman (\citeyear{hullman2011visualization}), cada dado absoluto é contextualizado com médias comparativas, reduzindo a possibilidade de interpretações manipuladas e estimulando conclusões informadas;
...
Para situar as iniciativas de transparência legislativa no espectro da participação cidadã, adotamos como referência analítica a ``Escada da Participação Cidadã'' (\textit{Ladder of Citizen Participation}), proposta por Arnstein (\citeyear{arnstein1969ladder}) em seu trabalho seminal. A tipologia organiza oito degraus de participação em três faixas, conforme o grau de poder efetivamente conferido aos cidadãos:

\begin{itemize}
    \item \textbf{Não-participação} (degraus 1--2): \textit{Manipulação} e \textit{Terapia} --- o objetivo real é ``educar'' ou ``curar'' os participantes, não ouvi-los. Comitês consultivos sem poder deliberativo exemplificam essa categoria;
    
    \item \textbf{Participação simbólica} (degraus 3--5): \textit{Informação}, \textit{Consulta} e \textit{Pacificação} --- cidadãos podem ouvir e ser ouvidos, mas sem garantia de que suas vozes influenciem decisões. Audiências públicas sem caráter vinculante e pesquisas de opinião situam-se neste nível;
...
Essa escala é especialmente útil para avaliar ferramentas de \textit{civic tech} (tecnologia cívica): ao classificar cada iniciativa em um degrau, torna-se possível identificar se a tecnologia efetivamente redistribui poder ou apenas simula participação. A seguir, aplicamos esse referencial para analisar as principais iniciativas de transparência legislativa no Brasil e no mundo.
...
Embora esses portais representem avanços significativos na transparência passiva, situam-se no \textbf{degrau 3 (Informação)} da Escada de Arnstein: disponibilizam dados brutos de forma unidirecional, sem mediação interpretativa nem canal de retorno ao cidadão. O cidadão comum, sem conhecimento técnico sobre APIs ou contabilidade pública, enfrenta barreiras substanciais para transformar dados dispersos em fiscalização efetiva. Em outras palavras, esses portais cumprem o requisito mínimo da transparência ativa, mas não avançam para os degraus de consulta ou pacificação.
...
\begin{itemize}
    \item \textbf{Rosie:} Algoritmo de aprendizado de máquina (\textit{machine learning}) desenvolvido em Python que audita a Cota para Exercício da Atividade Parlamentar (CEAP). Opera através de cinco classificadores principais: (1) \textit{meal price outlier} (valor de refeição atípico) --- identifica refeições com valores acima da média para o local; (2) \textit{irregular companies} (empresas irregulares) --- detecta gastos em empresas com situação cadastral irregular na Receita Federal; (3) \textit{traveled speeds} (velocidades de deslocamento) --- cruza gastos para identificar deslocamentos fisicamente impossíveis; (4) \textit{monthly subquota limit} (limite de subcota mensal) --- verifica excesso nos limites mensais por categoria; e (5) \textit{election expenses} (despesas eleitorais) --- identifica uso indevido da cota para financiar campanhas.
...
\begin{itemize}
    \item \textbf{Classificações Múltiplas:} Top 50 deputados com maiores gastos, classificação  de partidos por consumo da cota e lista de empresas fornecedoras --- incluindo filtro específico para ``empresas com sanções'' administrativas;
...
\begin{itemize}
    ...
    \item \textbf{Painel de Fornecedores:} Classificação das empresas que mais recebem recursos, identificando padrões de concentração de gastos.
\end{itemize}

Entretanto, as classificações do ``De Olho em Você'' baseiam-se em métricas agregadas diretas (quem mais gastou, quem mais enviou emendas), \textbf{sem metodologia explícita de efetividade legislativa}. Além disso, a plataforma não contempla o Senado Federal.

Na Escada de Arnstein, essas ferramentas situam-se entre os \textbf{degraus 3 e 4} --- \textit{Informação} e \textit{Consulta}. Diferentemente dos portais oficiais, que se limitam à disponibilização unidirecional de dados brutos, essas plataformas transformam informação pública em conhecimento acionável: alertas de anomalias, classificações comparativas e, no caso da Serenata de Amor, mecanismos de denúncia formal via Jarbas. Contudo, nenhuma delas institui canais de deliberação entre cidadãos e representantes, permanecendo aquém dos degraus de poder cidadão.
...
Ambas as plataformas operam predominantemente no \textbf{degrau 3 (Informação)} da escada, com o TheyWorkForYou aproximando-se do \textbf{degrau 4 (Consulta)} ao oferecer alertas personalizados que habilitam o monitoramento ativo e contínuo dos representantes pelo cidadão.
...
A exposição pública de critérios e pesos não é mera formalidade: representa um compromisso ético com a \textbf{transparência metodológica}. Diferente de classificações opacas, o cidadão poderá compreender --- e questionar --- os fundamentos da classificação, evitando que a plataforma seja percebida como veículo de viés político.

À luz da Escada de Arnstein, o \textit{Tô De Olho} posiciona-se entre os \textbf{degraus 3 e 5} --- \textit{Informação} e \textit{Pacificação}. A plataforma ultrapassa o degrau puramente informativo dos portais oficiais ao oferecer \textit{rankings} contextualizados, alertas de anomalias e comparativos visuais que empoderam o cidadão para exercer pressão informada sobre seus representantes. Contudo, por não incorporar mecanismos de deliberação vinculante (como consultas populares com efeito decisório), a ferramenta não alcança os degraus superiores de poder cidadão. Reconhecer essa limitação estrutural é fundamental: como adverte Arnstein (\citeyear{arnstein1969ladder}), ``participação sem redistribuição de poder é um processo vazio e frustrante para os desprovidos de poder''. O projeto assume, portanto, o papel de \textbf{pré-condição} para a participação efetiva, fornecendo a base informacional qualificada sem a qual os degraus superiores da escada são inatingíveis.
...
\begin{enumerate}
    ...
    \item \textbf{Inteligência de Dados}: Oferece alertas automáticos, classificações e visualizações que convertem dados brutos em informação acionável;
