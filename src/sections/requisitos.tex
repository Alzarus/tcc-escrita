...
\textbf{Módulo de \textit{Ranking} e \textit{Score}:}
\begin{itemize}
    \item \textbf{[RF22]} O sistema deve calcular e exibir o \textit{Score} de efetividade legislativa de cada senador, baseado no algoritmo de \textit{ranking} do projeto.
    ...
    \item \textbf{[RF24]} O sistema deve permitir ordenar e filtrar senadores por cada critério individual do \textit{ranking}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{[RNF12]} O sistema deve possuir \textit{pipelines} de CI/CD para integração e \textit{deploy} contínuos.
\end{itemize}
