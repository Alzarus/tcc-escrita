...
\begin{itemize}
    \item \textbf{[RF22]} O sistema deve calcular e exibir a pontuação (\textit{Score}) de efetividade legislativa de cada senador, baseado no algoritmo de \textit{ranking} do projeto.
    \item \textbf{[RF23]} O sistema deve apresentar gráfico radar exibindo as quatro dimensões do \textit{Score}: Produtividade Legislativa, Participação em Comissões, Presença em Votações e Economia na Cota.
...
\begin{itemize}
    ...
    \item \textbf{[RNF12]} O sistema deve automatizar ciclos de integração e entrega contínuas através de esteiras de CI/CD (\textit{pipelines}).
\end{itemize}
