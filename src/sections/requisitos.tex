...
Conforme destacado na análise de trabalhos relacionados (Seção 1.6), o \textit{Tô De Olho} sintetiza funcionalidades de plataformas consolidadas como \textit{De Olho em Você} e \textit{De Olho no Congresso}, adaptando-as ao contexto do Senado Federal. Os requisitos funcionais foram organizados em seis módulos:
...
% (Removido do escopo inicial - ver Trabalhos Futuros)
...
\textbf{Módulo de Classificação e Pontuação:}
\begin{itemize}
    \item \textbf{[RF22]} O sistema deve calcular e exibir a pontuação (\textit{Score}) de efetividade legislativa de cada senador, baseado no algoritmo de classificação (\textit{ranking}) do projeto.
    \item \textbf{[RF23]} O sistema deve apresentar gráfico radar com as quatro dimensões da pontuação (\textit{Score}): Produtividade Legislativa, Presença em Votações, Economia na Cota e Participação em Comissões.
    \item \textbf{[RF24]} O sistema deve permitir ordenar e filtrar senadores por cada critério individual da classificação.
\end{itemize}
...
\begin{itemize}
    \item \textbf{[RNF03]} O sistema deve ser acessível via navegadores \textit{web} em computadores de mesa (\textit{desktop}) e dispositivos móveis (\textit{mobile}).
    \item \textbf{[RNF04]} A interface deve seguir o padrão de prioridade para dispositivos móveis (\textit{mobile-first}) para garantir boa experiência em telas pequenas.
...
\begin{itemize}
    ...
    \item \textbf{[RNF11]} O código deve seguir padrões de desenvolvimento (análise estática, formatação) e estar documentado.
    \item \textbf{[RNF12]} O sistema deve possuir esteiras (\textit{pipelines}) de CI/CD para integração e implantação (\textit{deploy}) contínuos.
\end{itemize}
