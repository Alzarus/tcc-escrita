...
\begin{enumerate}
    \item \textbf{Estágio de Construção (\textit{Build})}: Utiliza uma imagem base completa (\texttt{golang:1.21-alpine}) contendo compiladores e ferramentas de construção necessárias para gerar o binário estático da aplicação.
    \item \textbf{Estágio de Execução (\textit{Runtime})}: Copia apenas o binário compilado para uma imagem minimalista (\texttt{gcr.io/distroless/static}), isenta de gerenciadores de pacotes ou \textit{shell}.
\end{enumerate}
