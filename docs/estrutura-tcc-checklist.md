...
- **Tecnologias adotadas**: Arquitetura de \textit{microservices} em Go; Next.js 15/TypeScript/Tailwind; PostgreSQL, Redis, RabbitMQ/PubSub; Docker/K8s; CI/CD GitHub Actions; fontes de dados (API v2 Câmara + arquivos em massa; API TSE para fórum).
- **Trabalhos Relacionados**: Portal da Transparência, Dados Abertos Câmara, Ranking dos Políticos, Serenata de Amor; diferenciar pelo foco em consolidação + fórum validado + UX acessível.
...
- **Modelo de Banco de Dados**: Descrição lógica/física se houver banco; relacionar com módulos (deputados, despesas, atividades, usuários, fórum).
...
- **Proposta de Solução**: [ ] Descrever plataforma e público-alvo; [ ] Funcionalidades-chave (perfil, despesas, votações, fórum validado TSE); [ ] Benefício para fiscalização cidadã.
- **Tecnologias adotadas**: [ ] Listar stack completo (Go, Next.js 15, PostgreSQL, Redis, RabbitMQ/PubSub, Docker/K8s, GitHub Actions); [ ] Fontes de dados (API v2 + arquivos em massa; API TSE); [ ] Motivar escolhas.
